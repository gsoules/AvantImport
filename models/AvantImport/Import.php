...
class AvantImport_Import extends Omeka_Record_AbstractRecord implements Zend_Acl_Resource_Interface
{
     ...
     *
     * @param AvantImport_ColumnMap_Set|array $maps The set of column maps
     * @throws InvalidArgumentException
     */
    public function setColumnMaps($maps)
    {
        if ($maps instanceof AvantImport_ColumnMap_Set) {
            $mapSet = $maps;
        } else if (is_array($maps)) {
            $mapSet = new AvantImport_ColumnMap_Set($maps);
        } else {
            throw new InvalidArgumentException("Maps must be either an "
                . "array or an instance of AvantImport_ColumnMap_Set.");
        }
     ...
     * AvantImport_Import and run start.
    ...
    /**
     * Returns the AvantImport_File object for the import.
     *
     * @return AvantImport_File
     */
     ...
     *
     * @throws UnexpectedValueException
     * @return AvantImport_ColumnMap_Set The set of column maps for the import
     */
    public function getColumnMaps()
    {
        if ($this->_columnMaps === null) {
            $columnMaps = unserialize($this->serialized_column_maps);
            if (!($columnMaps instanceof AvantImport_ColumnMap_Set)) {
                throw new UnexpectedValueException("Column maps must be "
                    . "an instance of AvantImport_ColumnMap_Set. Instead, the "
    ...
    {
        return $this->getTable('AvantImport_ImportedRecord')->getTotal($this->id);
    }
    ...
    protected function _undoImportLoop()
    {
        try {
            ...
            while ($importedRecords = get_records('AvantImport_ImportedRecord', array('import_id' => $this->id), $recordLimitPerQuery)) {
                ...
                foreach ($importedRecords as $importedRecord) {
                    ...
                    if ($batchSize > 0 && $deletedRecordsCount >= $batchSize) {
                        ...
                        foreach ($deletedRecords as $recordType => $recordIds) {
                            $db->delete($db->AvantImport_ImportedRecord, array(
    ...
    protected function _manageFromMappedRow()
    {
        ...
        $action = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_ACTION);
        // Avoid an empty choice by the user.
        if (empty($action)) {
            $action = AvantImport_ColumnMap_Action::DEFAULT_ACTION;
        }
        elseif ($action == AvantImport_ColumnMap_Action::ACTION_SKIP) {
            return AvantImport_ColumnMap_Action::ACTION_SKIP;
        }

        $identifierField = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_IDENTIFIER_FIELD);

        // Smart determination of the record type when empty
        $recordType = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_RECORD_TYPE, null);
        if (empty($recordType)) {
            // If there is a non empty column Collection, so this is an item.
            if (!empty($map[AvantImport_ColumnMap::TYPE_COLLECTION])) {
                $recordType = 'Item';
            }
            // If there is a non empty Item Type, this is an item.
            elseif (!empty($map[AvantImport_ColumnMap::TYPE_ITEM_TYPE])) {
                $recordType = 'Item';
            }
            // If there are multiple files, this is an item.
            elseif (isset($map[AvantImport_ColumnMap::TYPE_FILE])
                    && !is_null($map[AvantImport_ColumnMap::TYPE_FILE])
                    && count($map[AvantImport_ColumnMap::TYPE_FILE]) > 1
                 ) {
                $recordType = 'Item';
            }
            // If there is a non empty column Item, this is a file.
            elseif (!empty($map[AvantImport_ColumnMap::TYPE_ITEM])) {
                $recordType = 'File';
            }
            ...
            // By default, this is an item.
                ...
                $recordType = AvantImport_ColumnMap_RecordType::DEFAULT_RECORD_TYPE;
            }
        }
        ...
        $parentRecord = null;
        if ($recordType == 'File') {
            $itemIdentifier = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_ITEM);
            if ($itemIdentifier && isset($this->_defaultValues[AvantImport_ColumnMap::TYPE_IDENTIFIER_FIELD])) {
                $recordIdentifierField = $this->_defaultValues[AvantImport_ColumnMap::TYPE_IDENTIFIER_FIELD];
        ...
        if (empty($identifier) && empty($record) && $recordType == 'File') {
            $file = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_FILE);
        ...
        if ($recordType == 'File'
            ...
            $item = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_ITEM);
            if (!empty($item)) {
                $action = AvantImport_ColumnMap_Action::ACTION_CREATE;
            }
        }
        ...
        if ($record && $action == AvantImport_ColumnMap_Action::ACTION_CREATE) {
        ...
        switch ($action) {
            case AvantImport_ColumnMap_Action::ACTION_CREATE:
                switch ($recordType) {
                    case 'File':
            ...
            case AvantImport_ColumnMap_Action::ACTION_UPDATE_ELSE_CREATE:
                // As the record is present, "Update else Create" is "Update".
                $action = AvantImport_ColumnMap_Action::ACTION_UPDATE;
            case AvantImport_ColumnMap_Action::ACTION_UPDATE:
            case AvantImport_ColumnMap_Action::ACTION_ADD:
            case AvantImport_ColumnMap_Action::ACTION_REPLACE:
                // Allowed actions here are only the old ones.
                $record = $this->_updateRecord($record, $action);
                // Can't move this _updateRecord().
                $this->updated_record_count++;
                break;
            case AvantImport_ColumnMap_Action::ACTION_DELETE:
    ...
    protected function _addItemFromMappedRow()
    {
        ...
        $collectionId = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_COLLECTION);
        ...
        $elementTexts = $map[AvantImport_ColumnMap::TYPE_ELEMENT];
    ...
    protected function _addFileFromMappedRow()
    {
        ...
        $fileUrl = $map[AvantImport_ColumnMap::TYPE_FILE];
        ...
        $itemIdentifier = $map[AvantImport_ColumnMap::TYPE_ITEM];
        ...
        $this->_updateRecord($file, AvantImport_ColumnMap_Action::ACTION_ADD);
    ...
    protected function _getItemMetadataFromMappedRow()
    {
        ...
        $itemType = $this->_getMappedValue(AvantImport_ColumnMap::TYPE_ITEM_TYPE);
        ...
        $recordMetadata[Builder_Item::IS_PUBLIC] =
            $this->_getMappedValue(AvantImport_ColumnMap::TYPE_PUBLIC);
        $recordMetadata[Builder_Item::IS_FEATURED] =
            $this->_getMappedValue(AvantImport_ColumnMap::TYPE_FEATURED);
        $recordMetadata[Builder_Item::TAGS] =
            $this->_getMappedValue(AvantImport_ColumnMap::TYPE_TAG);
    ...
    protected function _getRecordFromIdentifier(
        $identifier,
        $recordType = AvantImport_ColumnMap_RecordType::DEFAULT_RECORD_TYPE,
        $identifierField = AvantImport_ColumnMap_IdentifierField::DEFAULT_IDENTIFIER_FIELD,
        $parentRecord = null
    ) {
        $record = false;
    ...
    {
        $avantImportRecord = new AvantImport_ImportedRecord();
        $avantImportRecord->setArray(array(
            'import_id' => $this->id,
            'record_type' => $recordType,
            'record_id' => $recordId,
            'identifier' => $identifier ?: '',
        ));
        $avantImportRecord->save();
        $this->_importedCount++;
    }
    ...
    protected function _getImportedRecord($identifier, $recordType = null, $currentImport = true)
    {
        ...
        $avantImportRecords = get_db()->getTable('AvantImport_ImportedRecord')
            ->findBy($bind, 1);
        if (!empty($avantImportRecords)) {
            $avantImportRecord = reset($avantImportRecords);
            $result = array(
                'record_type' => $avantImportRecord->record_type,
                'record_id' => $avantImportRecord->record_id,
            );
            return $result;
        }
    }
     ...
     *
     * @internal See AvantImport_ImportTask::_log()
     *
    ...
    private function _insertItem($metadata = array(), $elementTexts = array(), $fileMetadata = array(), $postData = array())
    {
        ...
        $result = $this->_setExtraData($record, $postData, AvantImport_ColumnMap_Action::ACTION_ADD);
        return $record;
    }
     ...
     *
     * @see AvantImport_Builder_Item::_addPostData()
     *
     * @param Record $record
     ...
     *
     * @see AvantImport_Builder_Item::_setPostDataViaSetArray()
     *
     * @param Record $record
    ...
    {
        // Always return true so that it's not necessary to configure security.ini.
        return true;

        $settings = Zend_Registry::get('avant_import');
